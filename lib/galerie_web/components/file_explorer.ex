defmodule GalerieWeb.Components.FileExplorer do
  ...
  def render(%{explorer: %Galerie.Explorer{parent: parent, items: items}} = assigns) do
    ...
    <div
      class={
        Html.class(
          "flex items-center px-2 py-1 rounded-md items-center",
          {is_nil(@parent), "text-gray-400", "hover:bg-gray-200 cursor-pointer"}
        )
      }
      phx-click={if not is_nil(@parent), do: @on_back}
    >
