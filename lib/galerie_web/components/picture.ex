defmodule GalerieWeb.Components.Picture do
  ...
  def grid(%{pictures: pictures} = assigns) do
    ...
    <div class="grid grid-cols-1 tablet:grid-cols-2 laptop:grid-cols-4 desktop:grid-cols-6 gap-4 m-auto mb-2">
      <%= for {index, picture} <- @filtered_pictures do %>
        <.thumbnail
          picture={picture}
          checked={SelectableList.index_selected?(@pictures, index)}
          index={index}
        />
      <% end %>
    </div>
  ...
  def thumbnail(assigns) do
    ...
    <div
      class={
        Html.class("relative transition cursor-pointer select-none group", {@checked, "scale-90"})
      }
      phx-click="picture-click"
    ...
    >
      <img
        class={
          Html.class(
            "h-full max-h-72 w-full rounded-md shadow-md border-4 group object-cover",
            {@checked, "border-pink-500", "border-true-gray-300"}
          )
        }
        src={~p(/pictures/#{@picture.id}?#{[type: "thumb"]})}
      />

      <div class={
        Html.class("w-full h-full group-hover:bg-gray-500/40 transition p-4 absolute z-1 top-0", [
          {not @checked, "opacity-0 group-hover:opacity-100"}
        ])
      }>
        <Ui.select_marker
          checked={@checked}
