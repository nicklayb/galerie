defmodule Nectarine.Changeset do
  ...
  def format_error({message, options}) do
    case Keyword.get(options, :count) do
      nil ->
        Gettext.dgettext(NectarineWeb.Gettext, "errors", message, options)

      count ->
        Gettext.dngettext(NectarineWeb.Gettext, "errors", message, message, count, options)
    end
  end
  ...
  ## Examples
      ...
      iex> generate_unique(changeset, :code, generator: Nectarine.Generator.Base64, length: 12)
  ...
  def generate_unique(%Ecto.Changeset{} = changeset, field, options) do
    ...
    value = Nectarine.Generator.unique(generator, options)
