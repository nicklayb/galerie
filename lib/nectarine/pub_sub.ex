defmodule Nectarine.PubSub do
  @moduledoc """
  Nectarine's pub sub module uses a custom Dispatcher to wrap
  every message in a `%Nectarine.PubSub.Message{}` structure
  to make them easily distinguishable from other mesages
  """
  alias Nectarine.PubSub.Message

  @dispatcher Nectarine.PubSub
  ...
  def topic(Nectarine.Project), do: "projects"
  def topic(Nectarine.User), do: "users"
  ...
  def broadcast(topic, message) when is_binary(topic) do
    Phoenix.PubSub.broadcast(Nectarine.PubSub, topic, {topic, message}, @dispatcher)
  end
  ...
  def subscribe(topic) when is_binary(topic) do
    Phoenix.PubSub.subscribe(Nectarine.PubSub, topic)
  end
